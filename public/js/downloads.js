...
let activeAnalyzeRequest = null;
let analyzeRequestSerial = 0;

function hashAnalyzeKeyPart(value) {
...
function buildAnalyzeKey(url, customCookies, cookiesBrowser) {
  return JSON.stringify({
    url,
    cookieMode: customCookies ? 'custom' : cookiesBrowser ? 'browser' : 'guest',
    cookiesBrowser: cookiesBrowser || '',
    cookieHash: customCookies ? hashAnalyzeKeyPart(customCookies) : ''
  });
}
  ...
  if (lastStatus === task.status && lastPending === currentPending) {
    ...
    if (meta) {
      ...
      if (task.errorMessage) {
        ...
        const isAuthError = errLower.includes('sign in') || errLower.includes('login') || errLower.includes('confirm your identity') || errLower.includes('confirm your age') || errLower.includes('not a bot') || errLower.includes('use --cookies') || errLower.includes('cookies-from-browser') || errLower.includes('private') || errLower.includes('members-only') || errLower.includes('unavailable') || errLower.includes('cookies');
        if (isYt && isAuthError) {
...
let lastTaskRenderFlushAt = 0;
const TASK_RENDER_MIN_INTERVAL_MS = 500;
...
function flushTaskRenders() {
  ...
  lastTaskRenderFlushAt = Date.now();

  for (const task of pendingTaskRenders.values()) {
    if (isCurrentTask(task)) {
      const activeCard = document.getElementById(`task-${task.id}`);
...
function scheduleTaskRender(task, immediate = false) {
  if (!task || !task.id) return;
  pendingTaskRenders.set(task.id, task);
  if (pendingRenderFrame || pendingRenderTimer) return;

  const elapsed = Date.now() - lastTaskRenderFlushAt;
  const delay = immediate ? 0 : Math.max(0, TASK_RENDER_MIN_INTERVAL_MS - elapsed);
...
function refreshTaskDisplay(task) {
  scheduleTaskRender(task, true);
}
...
function isYtdlpAuthMessage(message) {
...
function showAnalyzeCookieRetry(message, browser) {
  const badge = $('file-info-badge');
  const selectedBrowser = browser || appSettings.cookiesBrowser || 'chrome';
  badge.className = 'info-badge show warn cookie-retry-badge';
  badge.textContent = `X ${message} `;

  const bridgeBtn = document.createElement('button');
  bridgeBtn.type = 'button';
  bridgeBtn.className = 'analyze-retry-cookies-btn';
  bridgeBtn.textContent = 'Retry with extension cookies';
  bridgeBtn.addEventListener('click', (event) => retryAnalyzeWithBridgeCookies(event));
  badge.appendChild(bridgeBtn);

  const retryBtn = document.createElement('button');
  retryBtn.type = 'button';
  retryBtn.className = 'analyze-retry-cookies-btn';
  retryBtn.textContent = `Retry with ${selectedBrowser.toUpperCase()} cookies`;
  retryBtn.addEventListener('click', (event) => retryAnalyzeWithCookies(event, selectedBrowser));
  badge.appendChild(retryBtn);

  refreshCookieBridgeBadge();
}

async function retryAnalyzeWithCookies(event, browser) {
  if (event) event.preventDefault();
  const selectedBrowser = browser || appSettings.cookiesBrowser || 'chrome';
  await analyzeUrl({ cookiesBrowser: selectedBrowser, manualCookieRetry: true });
}

async function retryAnalyzeWithBridgeCookies(event) {
  if (event) event.preventDefault();
  await analyzeUrl({ useBridgeOnly: true, manualCookieRetry: true });
}

async function refreshCookieBridgeBadge() {
  const url = $('url-input').value.trim();
  if (!url) return;
  try {
    const res = await fetch(`/api/cookie-bridge/status?url=${encodeURIComponent(url)}`);
    if (!res.ok) return;
    const status = await res.json();
    const badge = $('file-info-badge');
    if (!badge || !badge.classList.contains('cookie-retry-badge')) return;
    const info = document.createElement('span');
    info.style.marginLeft = '8px';
    info.style.opacity = '0.85';
    info.textContent = status.available
      ? `(extension cookies ready: ${status.count || 'header'})`
      : '(extension cookies not loaded yet)';
...
async function analyzeUrl(options = {}) {
  ...
  const customCookies = $('adv-cookies')?.value.trim();
  const cookiesBrowser = options.useBridgeOnly ? undefined : (options.cookiesBrowser || (appSettings.cookiesEnabled ? appSettings.cookiesBrowser : undefined));
  const body = customCookies
    ? { url, cookiesHeader: customCookies, preferCookies: true }
    : cookiesBrowser ? { url, cookiesBrowser, preferCookies: !!options.manualCookieRetry } : { url, preferCookies: !!options.manualCookieRetry };
  const analyzeKey = buildAnalyzeKey(url, customCookies, cookiesBrowser);

  if (analyzedInfo && analyzedInfo._analyzeKey === analyzeKey) {
    $('btn-download').disabled = false;
    return analyzedInfo;
  }

  if (activeAnalyzeRequest && activeAnalyzeRequest.key === analyzeKey) {
    return activeAnalyzeRequest.promise;
  }

  const badge = $('file-info-badge');
  const btn = $('btn-analyze');
  const requestSerial = ++analyzeRequestSerial;
  btn.disabled = true;
  ...
  const runAnalyze = (async () => {
    try {
      ...
      if ($('url-input').value.trim() !== url || analyzeRequestSerial !== requestSerial) {
        return null;
      }

      analyzedInfo = data;
      analyzedInfo._analyzeKey = analyzeKey;

      if (data.isYtdlp) {
        setYtMode(true);
        if (data.cookieWarning) {
          badge.className = 'info-badge show warn';
          badge.textContent = t('warn_cookies_locked', 'Cookies locked: fallback loaded. Use extension cookies for restricted videos.');
        ...
        if (data.cookiesBrowser) analyzedInfo.cookiesBrowser = data.cookiesBrowser;
        analyzedInfo.ytdlpNoCookies = !!data.ytdlpNoCookies;
      ...
      if (details.isYtdlp && (details.canRetryWithCookies || details.authRequired || isYtdlpAuthMessage(err.message))) {
        setYtMode(true);
        showAnalyzeCookieRetry(err.message, details.cookiesBrowser || options.cookiesBrowser || appSettings.cookiesBrowser || 'chrome');
      } else {
        badge.className = 'info-badge show warn';
        badge.textContent = `X ${err.message}`;
      }
      $('btn-download').disabled = false;
    } finally {
      if (activeAnalyzeRequest && activeAnalyzeRequest.key === analyzeKey) {
        activeAnalyzeRequest = null;
      }
      if (analyzeRequestSerial === requestSerial) {
        btn.disabled = false;
  ...
  activeAnalyzeRequest = { key: analyzeKey, promise: runAnalyze };
  return runAnalyze;
}
...
async function startYtDownload(url, bypassSafety = false) {
  const taskUrl = analyzedInfo?.normalizedUrl || url;
  ...
  const customCookies = $('adv-cookies')?.value.trim();
  const ytdlpExtractorArgs = analyzedInfo?.ytdlpExtractorArgs || undefined;
  const ytdlpNoCookies = !!analyzedInfo?.ytdlpNoCookies;
  const cookiesBrowser = (ytdlpNoCookies || customCookies)
    ? undefined
    : (analyzedInfo?.cookiesBrowser || (appSettings.cookiesEnabled ? appSettings.cookiesBrowser : undefined));
  const cookiesHeader = ytdlpNoCookies ? undefined : (customCookies || undefined);

  try {
    const res = await fetch('/api/yt-tasks', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url: taskUrl, format, title, thumbnail, fileSize, speedLimitKbps, speedMode, cookiesBrowser, cookiesHeader, ytdlpExtractorArgs, ytdlpNoCookies, noCookies: ytdlpNoCookies, bypassSafety })
    });
    const data = await res.json();
    if (!res.ok) {
...
async function startHttpDownload(url, bypassSafety = false) {
  ...
  const segments = parseInt($('seg-count').value, 10) || appSettings.defaultSegments || 8;
  ...
  try {
    ...
    const limitStr = effectiveLimit > 0 ? ` (limited to ${formatKbps(effectiveLimit)})${quotaStr}${modeStr}` : quotaStr + modeStr;
    const queueStr = data.status === 'queued' && data.queuePosition ? ` (queued #${data.queuePosition})` : '';
    const effectiveConnections = data.connectionPolicy?.effectiveSegments || segments;
    const connectionLimitStr = data.connectionPolicy?.clamped ? ` (max ${data.connectionPolicy.maxConnections} for ${data.connectionPolicy.label})` : '';
    toast(t('toast_download_started', 'Download started!') + ` ${effectiveConnections} threads${connectionLimitStr}` + limitStr + queueStr, 'success');
